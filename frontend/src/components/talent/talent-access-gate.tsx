 ...
 *
 * 인재 정보(이름·나이·경력·자격증·희망조건)는 개인정보라 **승인 시설회원·보호자회원·관리자**만
 * 열람할 수 있다 (백엔드 `GET /api/jobseekers` = `hasAnyRole('FACILITY','ADMIN','GENERAL')` +
 * 시설은 승인 필요, 보호자회원은 승인 절차 없이 바로 허용). 비로그인·구직자·미승인 시설은
 * 안내 화면을 보여준다.
 */
...
function resolve(user: SessionUser | null, from: string) {
  ...
  // 구직자 / GUEST(유형 미선택)
  return {
    title: '시설 회원·보호자회원 전용 기능입니다.',
    description: '인재정보 열람은 승인된 시설 회원, 보호자회원, 관리자만 이용할 수 있습니다.',
    action: (
