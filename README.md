...
- 프로젝트 초기 셋업 — 모노레포 구조(`backend/`·`frontend/` 분리), CODEOWNERS, PR 자동 리뷰어 지정, GitHub Flow 협업 규칙(`CLAUDE.md`)
- 회원/인증/보안/고객센터 도메인, CORS 설정, JWT 로그인
- Render 배포 안정화 — 포트 바인딩, 스키마 검증, `@Lob`→`oid` 컬럼 문제, Flyway 마이그레이션 도입
- Cloudflare R2 파일 스토리지(presigned URL)
- 소셜 로그인 카카오 단일화(네이버/구글 숨김), 비밀번호 찾기(이메일/휴대폰 인증) API, 회원 탈퇴 API
- 알림 도메인(지원 결과/시설 승인/문의 답변) + 헤더 배지 폴링
- 관리자 인재 연락처 마스킹 해제, 결제/포인트 동시요청 이중 처리 버그 수정
- CI(GitHub Actions 빌드체크) 도입, README/트러블슈팅 문서 지속 관리

### 신영 (프론트엔드 · [@syyu21b](https://github.com/syyu21b))
- 회원가입 플로우(보호자/구직/시설 3단계, GENERAL role), 마이페이지, 약관·개인정보처리방침, 회사소개, 고객센터 콘텐츠
- 구인공고 목록/상세/홈/검색·필터(facets) 실 API 연동, "공고에 지원하기" 액션 + 등록/수정 폼
- 인재정보 목록/상세, 구직신청 실 API 연동
- 관리자 페이지(회원/포인트충전/시설/문의/공지 관리) 구현
- 포인트 충전 UI + 포트원(PortOne) 연동(구매자 이메일/휴대폰 전달, 미보유 시 입력창), 더미 데이터 대량 시딩
- 모바일 반응형 수정(가로 스크롤/텍스트 넘침)

### 경수 (백엔드 · [@HurKyungsoo](https://github.com/HurKyungsoo))
- 구인공고 도메인(등록/목록/상세/수정/삭제), 다중조건 검색·정렬·노출등급, 스크랩, 비슷한 공고, 매칭 스코어 계산
- 구직신청(Application) 도메인, 인재정보 검색 + 인재↔공고 매칭도, 구직자 프로필 필드 확장
- 공유 enum 통일 작업(직종/근무시간대/고용형태/시설유형/자격증종류) + `ENUM_MAPPING.md` 문서화
- 내 주변 일자리 — nearby 반경검색 API, 지도+리스트+카카오맵, 반경 실시간 추적/클러스터링/드래그, 마커 공유(카카오톡/링크), 프리미엄 공고 강조
- 인재 이름 마스킹 서버측 강제, 인재정보 접근 게이트(승인 시설회원·관리자 전용)

### 동한 (프론트엔드 · [@Kim-dong-han](https://github.com/Kim-dong-han))
- 프론트엔드 프로젝트 초기화 + 홈 화면
- 구인공고 목록/상세, 인재정보 목록/상세, 회원가입, 구직신청, 고객센터, 내 주변 일자리 페이지 구현
- 상단 메뉴 회원 유형별 재설정, 관리자 계정 구직신청 화면 열람 허용, 회원가입 휴대폰 번호 하이픈 자동입력
