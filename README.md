...
- **구인공고** — 등록·수정·마감, 목록·상세, 추천순/최신순/매칭점수순 정렬, 내 주변 일자리(반경 검색 + 지도 클러스터링), 임시저장, 스크랩
- **인재정보** — 구직자 프로필, 인재 검색, 공고↔인재 매칭도 계산
- **인증구직자 마크** — 구직자가 승인된 자격증·경력인증(증빙 파일 첨부)을 각 1건 이상 보유하면 마크 신청 가능, 관리자 최종 승인 시 부여(또는 회원관리에서 수동 부여/해제). 인재 목록/상세에 마크 표시
...
- **버그 수정**: 결제/포인트 동시요청 이중 처리, 문의 답변 등록 응답 null id, 매칭점수순 정렬이 비구직자에게 노출등급 우선으로 되돌아가던 문제
- **관리자**: 회원관리에서 인증구직자 마크 수동 부여/해제(정상 심사 절차 우회, 운영 편의용)
- **구인공고**: 정렬에 매칭점수순(`MATCH_SCORE`) 추가 — 노출등급과 무관하게 순수 매칭점수로만 정렬
...
- **경력 인증**: 증빙 파일 첨부 추가(텍스트만 받던 것 → 실제 서류 업로드)
- **구직신청**: 근무 시간대 선택 시 희망 근무시간 자동 채움, 등록 실패("요청 값이 올바르지 않습니다") 버그 수정
- **회원가입**: 아이디·이메일 중복 시 알림 문구 명확화
