...
`CareerBucket` = `ENTRY/Y1_3/Y3_5/Y5_PLUS`.
...
| 메서드 | 경로 | 권한 | 설명 |
|---|---|---|---|
| POST | `/api/job-postings/{id}/applications` | JOBSEEKER | 공고에 지원. 취소 이력 있으면 되살림. 201 `{applicationId}` |
| GET | `/api/job-postings/{id}/applications` | 공고 작성 시설 | 지원자 목록 (`PageResponse<ApplicantResponse>`, `?status=` 필터) |
| GET | `/api/members/me/applications` | JOBSEEKER | 내 지원 내역 (`PageResponse<MyApplicationResponse>`, `?status=` 필터) |
| PATCH | `/api/applications/{id}/cancel` | 지원자 본인 | 지원 취소 (APPLIED → CANCELED, 204) |
