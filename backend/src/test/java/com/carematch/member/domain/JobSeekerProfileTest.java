...
class JobSeekerProfileTest {
    ...
    void 희망근무조건_수정_저장된다() {
        ...
        profile.updateDesiredConditions(new JobSeekerProfile.DesiredConditions(
                JobType.CAREGIVER, WorkType.COMMUTE, WorkSchedule.DAY,
                List.of(new DesiredRegion("서울특별시", "강남구")), PayType.MONTHLY, 2_500_000));
        ...
        assertThat(profile.getDesiredRegions())
                .containsExactly(new DesiredRegion("서울특별시", "강남구"));
    ...
    void 희망근무조건_일부_null_이면_조건없음으로_덮어쓴다() {
        ...
        assertThat(profile.getDesiredRegions()).isEmpty();
